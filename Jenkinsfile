	...
	stages {
		...
		stage('Run container based on builded image') {
			...
			steps {
				script {
					sh '''
					docker run --name $IMAGE_NAME -d -p 80:5000 --network $TEAM5_NETWORK -v /var/lib/docker/volumes/jenkins-training_jenkins_home/_data/workspace/deploiement-miniprojet/simple_api/student_age.json:/data/student_age.json $IMAGE_NAME:$IMAGE_TAG
					sleep 5
					'''
				}
			}
